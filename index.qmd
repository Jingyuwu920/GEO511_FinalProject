...
# Introduction I like how you narrow the focus here to desire lines vs. existing vs. optimized routes. Very clear research question.
...
flow_all <- expand.grid(o = shelters_sf_fix$id, #create origin
  ...
  sample_n(50) #sample 50 random paris to avoid blowing up the computer This block is very clear and clever. I learned how to use expand.grid() plus filter(o != d) to generate all O–D pairs and then downsample with sample_n() to keep the computation manageable. I hadn’t thought of this workflow before.
...
#if route found, return the route Great use of try() and returning NULL for failed routes. This makes your later map2() call much more robust and avoids the whole pipeline breaking if OSRM can’t find a path.
  return(r)
}
